...
import torch
import random                       # Random variables 관련 함수
import matplotlib.pyplot as plt     # 그래프 출력 관련 함수
from collections import deque       # Episode 기록 관련 함수
...
def bot_play(mainDQN):
    ...
    while True:
        ...
        if done or step >= 5000:  # 막대가 쓰러지거나 5000번 이상 진행시 종료
            print("Total score: {}".format(step))
            break

env = gym.make('CartPole-v0')   # CartPole 환경 생성
env._max_episode_steps = 10000  # 1 에피소드당 에피소드 최대 스탭 설정

dis = 0.9                   # 감가율 (Discounting factor)
e = 1                       # E-greedy 정책을 위한 Epsilon
e_decay = 0.99              # 상기 Epsilon 의 감소율
...
with tf.Session() as sess:
    ...
    sess.run(copy_ops)  # Main DQN과 Target DQN 동기화 실행
    ...
    for episode in range(max_episodes): # 에피소드 루프 시작
        ...
        while not done:
            ...
            if done: # 종료시 마지막 행동에 대한 Penalty
                reward = -100
        ...
        print("Episode: {} steps: {}, e: {}".format(episode, step_count, e)) # 에피소드 당 스탭 수 및 epsilon 출력

        if np.mean(score_history[-min(10, len(score_history)):]) >= 10000: # 최근 10회 에피소드의 평균 스코어가 10000 이상이면,
            bot_play(mainDQN)                                              # Cart pole 애니메이션 실행 후
            break                                                          # 프로그램 종료
        elif np.mean(score_history[-min(10, len(score_history)):]) >= 10 and beginner == False: # 평균 점수가 10 이상이면,
            bot_play(mainDQN)
            beginner = True # 초심자!
        elif np.mean(score_history[-min(10, len(score_history)):]) >= 500 and intermediate == False : # 평균 점수가 500 이상이면,
            bot_play(mainDQN)
            intermediate = True # 중급자!
